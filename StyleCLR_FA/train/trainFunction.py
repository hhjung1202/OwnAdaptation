...
def train(args, state_info, Train_loader, Test_loader, epoch):
    ...
    for it, (x, y, label) in enumerate(Train_loader):
        ...
        style_loss, content_loss = state_info.forward(x)
        total_loss = 1e-9

        if args.loss[0] is 1: total_loss += style_loss * args.weight[0];
        if args.loss[1] is 1: total_loss += content_loss * args.weight[1];

        total_loss.backward()
        state_info.optim_model.step()

        if it % 10 == 0:
            utils.print_log('Train, {}, {}, {:.6f}, {:.9f}, {:.9f}'.format(epoch, it
                , total_loss.item(), style_loss.item(), content_loss.item()))
            print('Train, {}, {}, {:.6f}, {:.9f}, {:.9f}'.format(epoch, it
                , total_loss.item(), style_loss.item(), content_loss.item()))
