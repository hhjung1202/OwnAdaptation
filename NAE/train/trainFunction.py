...
def train_step3_post(args, state_info, Train_loader, Test_loader, Memory, criterion, epoch):
    cuda = True if torch.cuda.is_available() else False
    FloatTensor = torch.cuda.FloatTensor if cuda else torch.FloatTensor
    LongTensor = torch.cuda.LongTensor if cuda else torch.LongTensor
    ...
    # train
    state_info.model.train()
    utils.print_log('Type, Epoch, Batch, total, Cls, Noise_V, Random_V, Pseudo_V, Regular, Noise%, Pseudo%, Real%, Pseu_Nois%, Pseu_Real%')
    for it, (x, y, label) in enumerate(Train_loader):
        ...
        pseudo_label = Memory.Calc_Pseudolabel(z)
        Memory.Batch_Insert(z, pseudo_label)

        loss_P = Memory.get_DotLoss_Noise(z, pseudo_label, reduction="mean", reverse=False)
        loss_N = Memory.get_DotLoss_Noise(z, y, reduction="mean", reverse=False)
        loss_R = Memory.get_DotLoss_Noise(z, rand_y, reduction="mean", reverse=True)    
        ...
        total = args.t0 * loss + args.t1 * loss_N + args.t2 * loss_R + args.t4 * loss_P + reg    
        total.backward()
        state_info.optim_model.step()
        ...
        if it % 10 == 0:
            utils.print_log('Train, {}, {}, {:.6f}, {:.6f}, {:.6f}, {:.6f}, {:.6f}, {:.6f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}'
                  .format(epoch, it, total.item(), loss.item(), loss_N.item(), loss_R.item(), loss_P.item(), reg.item()
                    , 100.*correct_Noise / train_Size
                    , 100.*correct_Pseudo / train_Size
                    , 100.*correct_Real / train_Size
                    , 100.*Pseudo_Noise / train_Size
